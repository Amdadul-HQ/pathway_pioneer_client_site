    ...
    return (
        <section className="max-w-[1440px] mx-auto my-32">
            ...
            <div className="grid grid-cols-3 gap-6 mt-5">
                {
                    touristsSpots && touristsSpots.map(spot => <div key={spot.id} className="p-5 border rounded-xl flex flex-col justify-between">
                    <div>
                        <img className="w-full h-[180px] md:h-[250px] rounded-xl" src={spot.photourl} alt="" />
                    </div>
                    <p className="text-base text-[#6E00FF] mt-2 font-medium">{spot.country}</p>
                    <h1 className="text-[#010101] text-xl md:text-3xl mb-4">{spot.tourists_spot_name}</h1>
                    ...
                    <div className="flex justify-between items-center text-[#010101] text-xl mt-3 font-medium">
                        <p>For {spot.average_cost}</p>
                        <p>Price: {spot.totalVisitorsPerYear}</p>
                    </div>
                    <Link to={`/spot/${spot._id}`} className=" mt-3 backdrop-blur-3xl relative inline-flex items-center px-12 py-3 overflow-hidden text-lg font-medium text-black border-2 border-black bg-gray-50 rounded-full hover:text-white group ">
